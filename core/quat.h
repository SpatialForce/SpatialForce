...
struct quat_t {
    ...
    inline CUDA_CALLABLE quat_t(Type x = Type(0), Type y = Type(0), Type z = Type(0), Type w = Type(0))
        : x(x), y(y), z(z), w(w) {}
    explicit inline CUDA_CALLABLE quat_t(const vec_t<Type, 3> &v, Type w = Type(0)) : x(v[0]), y(v[1]), z(v[2]), w(w) {}
...
template<typename Type>
inline CUDA_CALLABLE quat_t<Type> quat_from_axis_angle(const vec_t<Type, 3> &axis, Type angle) {
    Type half = angle * Type(Type(0.5));
    Type w = cos(half);

    Type sin_theta_over_two = sin(half);
    vec_t<Type, 3> v = axis * sin_theta_over_two;

    return quat_t<Type>(v[0], v[1], v[2], w);
}

template<typename Type>
inline CUDA_CALLABLE void quat_to_axis_angle(const quat_t<Type> &q, vec_t<Type, 3> &axis, Type &angle) {
    vec_t<Type, 3> v = vec_t<Type, 3>(q.x, q.y, q.z);
...
template<typename Type>
inline CUDA_CALLABLE vec_t<Type, 3> quat_rotate(const quat_t<Type> &q, const vec_t<Type, 3> &x) {
    Type c = (Type(2) * q.w * q.w - Type(1));
    Type d = Type(2) * (q.x * x.c[0] + q.y * x.c[1] + q.z * x.c[2]);
    return vec_t<Type, 3>(x.c[0] * c + q.x * d + (q.y * x[2] - q.z * x[1]) * q.w * Type(2),
                          x.c[1] * c + q.y * d + (q.z * x[0] - q.x * x[2]) * q.w * Type(2),
                          x.c[2] * c + q.z * d + (q.x * x[1] - q.y * x[0]) * q.w * Type(2));
}

template<typename Type>
inline CUDA_CALLABLE vec_t<Type, 3> quat_rotate_inv(const quat_t<Type> &q, const vec_t<Type, 3> &x) {
    Type c = (Type(2) * q.w * q.w - Type(1));
    Type d = Type(2) * (q.x * x.c[0] + q.y * x.c[1] + q.z * x.c[2]);
    return vec_t<Type, 3>(x.c[0] * c + q.x * d - (q.y * x[2] - q.z * x[1]) * q.w * Type(2),
                          x.c[1] * c + q.y * d - (q.z * x[0] - q.x * x[2]) * q.w * Type(2),
...
template<typename Type>
inline CUDA_CALLABLE mat_t<Type, 3, 3> quat_to_matrix(const quat_t<Type> &q) {
    vec_t<Type, 3> c1 = quat_rotate(q, vec_t<Type, 3>(1.0, 0.0, 0.0));
    vec_t<Type, 3> c2 = quat_rotate(q, vec_t<Type, 3>(0.0, 1.0, 0.0));
    vec_t<Type, 3> c3 = quat_rotate(q, vec_t<Type, 3>(0.0, 0.0, 1.0));

    return mat_t<Type, 3, 3>(c1, c2, c3);
}

template<typename Type>
inline CUDA_CALLABLE quat_t<Type> quat_from_matrix(const mat_t<Type, 3, 3> &m) {
...
inline CUDA_CALLABLE mat_t<Type, Rows, Cols>::mat_t(const vec_t<Type, 3> &pos,
                                                    const quat_t<Type> &rot,
                                                    const vec_t<Type, 3> &scale) {
    mat_t<Type, 3, 3> R = quat_to_matrix(rot);
