...
struct shape_t {
    ...
    CUDA_CALLABLE [[nodiscard]] inline int size() const {
...
struct array_t {
    ...
    CUDA_CALLABLE [[nodiscard]] inline bool empty() const { return !data; }
...
struct indexedarray_t {
    ...
    CUDA_CALLABLE [[nodiscard]] inline bool empty() const { return !arr.data; }
...
inline CUDA_CALLABLE T atomic_add(const A<T> &buf, int i, T value) {
...
template<template<typename> class A, typename T>
inline CUDA_CALLABLE T atomic_sub(const A<T> &buf, int i, T value) {
    return atomic_add(&index(buf, i), -value);
}
template<template<typename> class A, typename T>
...
template<template<typename> class A, typename T>
inline CUDA_CALLABLE T load(const A<T> &buf, int i) {
    return index(buf, i);
}
template<template<typename> class A, typename T>
inline CUDA_CALLABLE T load(const A<T> &buf, int i, int j) {
    return index(buf, i, j);
}
template<template<typename> class A, typename T>
inline CUDA_CALLABLE T load(const A<T> &buf, int i, int j, int k) {
    return index(buf, i, j, k);
}
template<template<typename> class A, typename T>
inline CUDA_CALLABLE T load(const A<T> &buf, int i, int j, int k, int l) {
...
CUDA_CALLABLE inline T2 select(const array_t<T1> &arr, const T2 &a, const T2 &b) {
    return arr.data ? b : a;
}
