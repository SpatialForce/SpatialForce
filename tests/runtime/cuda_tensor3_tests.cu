...
TEST_F(CudaTensor3Test, Constructors) {
    ...
    {
        CudaTensor3<float> arr(CudaStdArray<size_t, 3>(3, 7, 4));
    ...
    {
        CudaTensor3<float> arr(CudaStdArray<size_t, 3>(1, 9, 5), 1.5f);
    ...
    {
        CudaTensor3<float> arr(3, 4, 2, 7.f);
        EXPECT_EQ(3u, arr.width());
