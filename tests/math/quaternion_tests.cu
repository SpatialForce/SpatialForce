...
CUDA_TEST(Quaternion, Constructors) {
    {
        QuaternionD q;
    ...
    }
    {
        ...
        EXPECT_NEAR(originalAngle, angle, 1.0e-10);
    }

    {
        ...
        EXPECT_NEAR(pi<double>() / 2.0, angle, 1.0e-10);
    }
    {
        ...
        EXPECT_NEAR(std::sqrt(2.0) / 2.0, q.w, 1.0e-10);
        EXPECT_NEAR(std::sqrt(2.0) / 2.0, q.x, 1.0e-10);
        EXPECT_DOUBLE_EQ(0.0, q.y);
        EXPECT_DOUBLE_EQ(0.0, q.z);
    }
}

CUDA_TEST(Quaternion, BasicSetters) {
    {
        QuaternionD q;
...
CUDA_TEST(Quaternion, Normalized) {
    QuaternionD q(1, 2, 3, 4);
...
CUDA_TEST(Quaternion, BinaryOperators) {
...
CUDA_TEST(Quaternion, Modifiers) {
...
CUDA_TEST(Quaternion, ComplexGetters) {
    QuaternionD q(1, 2, 3, 4);
    ...
    for (int i = 0; i < 9; ++i) {
        EXPECT_NEAR(solution3[i], mat3[i], 1.0e-10);
    }
    ...
    for (int i = 0; i < 16; ++i) {
        EXPECT_NEAR(solution4[i], mat4[i], 1.0e-10);
    }
...
CUDA_TEST(Quaternion, SetterOperators) {
    QuaternionD q(1, 2, 3, 4);
...
CUDA_TEST(Quaternion, GetterOperators) {
    QuaternionD q(1, 2, 3, 4);
