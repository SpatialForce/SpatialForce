...
CUDA_TEST(Vector3, Constructors) {
    Vector3F vec;
...
CUDA_TEST(Vector3, BasicSetterMethods) {
...
CUDA_TEST(Vector3, BasicGetterMethods) {
...
CUDA_TEST(Vector3, BracketOperators) {
...
CUDA_TEST(Vector3, AssignmentOperators) {
...
CUDA_TEST(Vector3, AugmentedOperators) {
...
CUDA_TEST(Vector3, EqualOperatators) {
...
CUDA_TEST(Vector3, MinMaxFunctions) {
    Vector3F vec(5.f, 1.f, 0.f);
    Vector3F vec2(3.f, 3.f, 3.f);
...
CUDA_TEST(Vector3, ClampFunction) {
...
CUDA_TEST(Vector3, CeilFloorFunctions) {
...
CUDA_TEST(Vector3, BinaryOperators) {
    Vector3F vec(3.f, 9.f, 4.f);
    vec = vec + 4.f;
    EXPECT_FLOAT_EQ(7.f, vec.x);
    EXPECT_FLOAT_EQ(13.f, vec.y);
    ...
    EXPECT_NEAR(thrust::get<0>(tangential).dot(normal), 0.0, 1e-9);
    EXPECT_NEAR(thrust::get<1>(tangential).dot(normal), 0.0, 1e-9);
}
