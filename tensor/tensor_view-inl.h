...
template<typename T, size_t N>
TensorView<T, N>::TensorView(T *ptr, const Vector<size_t, N> &shape_)
    : TensorView() {
    Base::setPtrAndShape(ptr, shape_);
}

template<typename T, size_t N>
template<size_t M>
TensorView<T, N>::TensorView(typename std::enable_if_t<(M == 1), T *> ptr, size_t size_)
...
template<typename T, size_t N>
TensorView<const T, N>::TensorView(const T *ptr, const Vector<size_t, N> &shape_)
    : TensorView() {
    Base::setPtrAndShape(ptr, shape_);
}
...
template<typename T, size_t N>
template<size_t M>
TensorView<const T, N>::TensorView(typename std::enable_if_t<(M == 1), const T *> ptr, size_t size_)
    : TensorView(ptr, Vector<size_t, N>{size_}) {}
